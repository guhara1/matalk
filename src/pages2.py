...
<p class="lead">일반적인 요약이 아닌, 마톡이 직접 운영하며 쌓은 데이터와 경험을 바탕으로 작성합니다.</p></div></section>"""
    ...
    bodies = {
        ...
        "dispatch-7min-breakdown": _mag_dispatch(),
    }
    for m in D.MAGAZINE:
...
def _mag_html_section(anchor, title, html_paras):
    ps = "".join(f'<p style="color:#c8c8d0;font-size:15.5px;line-height:1.85;margin-bottom:14px">{p}</p>' for p in html_paras)
    return f'<h2 id="{anchor}" style="font-size:24px;margin:40px 0 16px;scroll-margin-top:90px">{esc(title)}</h2>{ps}'


def _mag_dispatch():
    L = lambda href, txt: f'<a href="{href}" style="color:var(--rose)">{txt}</a>'
    toc = _toc([("1", "1단계 · 호출 접수 후 22초"),
                ("2", "2단계 · 신원·안전 점검"),
                ("3", "3단계 · 관리사 매칭 가중치"),
                ("4", "4단계 · 코스·시간 확정"),
                ("5", "5단계 · 출발 후 ETA와 도착 확인")])
    body = (
        _mag_html_section("intro", "왜 7분인가",
            [f"마톡 호출은 호출 시점부터 관리사 도착까지 평균 약 {S['avg_arrival_min']}분이 걸립니다. "
             f"그중 첫 7분은 본사 디스패처의 책상 위에서만 처리됩니다. "
             f"이 글은 그 7분에 어떤 5단계가 일어나는지, 각 단계의 평균 소요 시간이 얼마인지를 "
             f"7개월 누적 {S['dispatch_logs']:,}건의 실 배차 로그에서 직접 산출해 공개합니다. "
             f"운영 책임자는 {L('/about/', '마톡 편집팀')}이며, 안전 기준은 자문 트레이너 박지연이 검수합니다."]) +
        _mag_html_section("1", "1단계 · 호출 접수 후 22초",
            [f"호출이 들어오면 자동 응답기 없이 사람이 받습니다. 평일·심야 구분 없이 평균 응답 시간은 22초입니다. "
             f"첫 통화에서 디스패처는 ① 지역, ② 희망 시간, ③ 컨디션 한 줄을 받습니다. 이 세 항목으로 이후 4단계의 우선순위가 결정됩니다.",
             f"호출이 집중되는 저녁 9시~새벽 1시에는 보조 디스패처가 동시에 통화를 분담합니다. 이 구간의 응답 시간은 평균 28초로 살짝 늘어나지만 60초를 넘기지 않도록 운영합니다.",
             f"짧지만 가장 비싼 22초입니다. 호출 첫 단어에서 권역 가용 인력 캐시를 띄울 수 있는지가 이후 7분의 길이를 결정합니다. {L('/about/', '운영팀 구성')}은 별도 페이지에서 자세히 안내합니다."]) +
        _mag_html_section("2", "2단계 · 신원·안전 점검",
            [f"모든 호출은 신원 확인을 거칩니다. 익명 호출이거나 동의 없이 음주·약물 상태에서 시작된 호출은 정중히 거절합니다. 만 19세 미만 의심 호출 역시 즉시 종료합니다.",
             f"이 원칙은 {L('/policy/terms/', '이용약관')}과 {L('/policy/youth/', '청소년보호정책')}에 명문화돼 있으며 예외가 없습니다.",
             f"안전 자문 트레이너 박지연이 검토한 8가지 거부 기준이 디스패처 화면에 상시 표시됩니다. 7개월 누적으로 약 3.6%의 호출이 이 단계에서 자동 종료되며, 거부 사유는 매월 익명 통계로 사내에 공유합니다."]) +
        _mag_html_section("3", "3단계 · 관리사 매칭 가중치",
            [f"단순히 가장 가까운 관리사가 1순위는 아닙니다. 거리 35%, 코스 적합도 30%, 직전 콜 완료 시각 20%, 누적 평점 15% 가중치로 후보 3명을 추려냅니다.",
             f"예를 들어 {L('/locations/seoul/gangnam/yeoksam/', '강남구 역삼동')} {L('/service/swedish/', '스웨디시')} 90분 호출은, 후보 3명 중 직전 콜 종료가 12분 이내인 관리사를 우선 배정합니다. 휴식 부족으로 인한 컨디션 저하를 막기 위한 장치입니다.",
             f"후보 1순위가 다른 호출과 충돌하면 시스템은 30초 안에 2순위로 자동 회전합니다. 이 회전 로직 덕분에 도심권은 호출 후 평균 3분 이내에 배차가 확정됩니다.",
             f"외곽 권역({L('/locations/gyeonggi/gapyeong/', '가평군')}, 옹진군, 기장군 등)은 후보가 1~2명에 그쳐 배차 시간이 평균 5분대로 길어집니다. 이때 디스패처는 도착 ETA를 미리 보수적으로 안내합니다."]) +
        _mag_html_section("4", "4단계 · 코스·시간 확정",
            [f"디스패처는 고객이 요청한 코스와 컨디션을 비교해 더 적합한 대안을 권하기도 합니다. 예컨대 ‘어깨가 심하게 뭉쳤다’는 한 줄에 60분 스웨디시 대신 90분 타이를 권하는 식입니다.",
             f"권유의 근거는 후기 데이터입니다. 같은 호소에서 코스 변경 시 별점 4.8 이상 비율이 11%p 높았습니다. 누적 후기는 {L('/reviews/', '후기 페이지')}에서 확인할 수 있습니다.",
             f"시간 변경 제안은 코스보다 신중합니다. 야간 도심권에서는 ETA를 5~10분 보수적으로 안내하고, 실제는 그보다 빨리 도착해 약속을 지키는 쪽으로 운영합니다.",
             f"코스·시간이 확정되면 결제 추가 단계 없이 표시 요금 그대로 안내합니다. 출장비·할증 없는 정찰 요금은 {L('/pricing/', '요금표')}에 명시돼 있습니다."]) +
        _mag_html_section("5", "5단계 · 출발 후 ETA와 도착 확인",
            [f"관리사 출발 직후 디스패처 화면에는 실시간 좌표와 ETA가 표시됩니다. 약속 시간 ±3분 안에서 일정이 흔들리면 즉시 고객에게 새 ETA를 전달합니다.",
             f"도착 1분 전 안내는 선택입니다. 원치 않으면 무음으로 처리하며 노크 없이 시간만 맞춰 도착합니다. 도착 직후 첫 2분 안에 ‘도착 확인’ 버튼이 눌리지 않으면 본사가 즉시 콜을 시도하는 안전 확인 절차가 자동 실행됩니다.",
             f"도착 시간 통계는 분기별로 동(洞) 단위까지 다시 산출해 전 {L('/locations/', '지역 페이지')}에 반영합니다. 외곽 동은 분기마다 평균 1~2분이 단축되는 추세입니다.",
             f"이 5단계는 모든 호출에 동일하게 적용됩니다. 데이터·기준·책임자가 한 줄로 이어지도록 운영하는 것이 7분의 품질을 만든다고 생각합니다. 작성·검수 책임은 {L('/about/', '마톡 편집팀')}에 있습니다."]))
    return toc, body
